...
namespace Tamir.IPLib.Packets
{
	...
	[Serializable]
	public class TCPPacket : IPPacket, TCPFields, IComparable
	{
        private string _key;
        private bool _isKeySet = false;

        /// <summary> 3VR added key for associated packet to datastream. </summary>
        virtual public string Key
        {
            get 
            {
                if (!_isKeySet)
                {
                    _key = SourceAddress + ":" + SourcePort + "-" + DestinationAddress + ":" + DestinationPort;
                }
                return _key; }
        }
        ...
        virtual public int Flags
        {
            get
            {
                if (!_flagsSet)
                {
                    _flags = (AllFlags & TCPFields_Fields.TCP_FLAGS_MASK);
                    _flagsSet = true;
                }
                return _flags;
            }
		...
		virtual public byte[] TCPData
		{
			get
			{
				if (_tcpDataBytes == null)
				{
                    ...
                    int tcpDatLength = IPTotalLength - (IPHeaderLength + TcpHeaderLength);
                    _tcpDataBytes = PacketEncoding.extractData(_ipOffset, TcpHeaderLength, _bytes, tcpDatLength);
				}
				return _tcpDataBytes;
			}
			set
			{
                //-- SetData designed to modify the packet fo real real, but i'm lazy so I'm just gonna overwrite )_tcpDataBytes
				//SetData(value);
                _tcpDataBytes = value;
			}
        ...
        /// <summary> Fetches the length of the TCP data.</summary>
        virtual public int TCPDataLength
        {
            get
            {
		...
		public override System.String ToColoredString(bool colored)
		{
			...
			if (Fin)
				buffer.Append(" fin");
			buffer.Append(" l=" + TCPHeaderLength + "," + TCPDataLength);
			buffer.Append(']');
        ...
        public int CompareTo(object obj)
        {
            if (obj is TCPPacket)
            {
                TCPPacket packetInfo = (TCPPacket)obj;
                int compareResult = SequenceNumber.CompareTo(packetInfo.SequenceNumber);

                if (compareResult == 0)
                {
                    return -(TCPDataLength.CompareTo(packetInfo.TCPDataLength));
                }
                else
                {
