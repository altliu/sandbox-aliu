...
namespace Tamir.IPLib.Packets
{
	...
	public class PacketFactory
	{
		...
		public static Packet dataToPacket(int linkType, byte[] bytes, Timeval tv)
		{
            ...
            if (ethProtocol <= 1500 && ethProtocol > 45)
            {
                //added by jscoles@3vr.com 6/7/2010
                //this is an 802.3 or 802.2 packet, the eth protocol is a length, not an ID
                //http://wiki.wireshark.org/Ethernet
                try
                {
                    return new SNAPacket(lLen, bytes);    
                }
                catch (Exception)
                {
